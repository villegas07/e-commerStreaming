...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "INSERT INTO LoMejorParaTi (nombre, precio, garantia, descripcion, imagen_url) VALUES ('$nombre', '$precio', '$garantia', '$descripcion', '$imagenPath')";
...
if (isset($_POST['editar'])) {
    $id = $_POST['id'];
    $resultado = $conexion->query("SELECT * FROM LoMejorParaTi WHERE id=$id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "UPDATE LoMejorParaTi SET nombre='$nombre', precio='$precio', garantia='$garantia', descripcion='$descripcion', imagen_url='$imagenPath' WHERE id=$id";
        }
    } else {
        $query = "UPDATE LoMejorParaTi SET nombre='$nombre', precio='$precio', garantia='$garantia', descripcion='$descripcion' WHERE id=$id";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar'])) {
    $id = $_POST['id'];
    $query = "DELETE FROM LoMejorParaTi WHERE id=$id";
    if ($conexion->query($query) === TRUE) {
...
$elementos = $conexion->query("SELECT * FROM LoMejorParaTi");
?>
...
<head>
    ...
    <title>Gestión de Lo Mejor Para Ti</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
    <div class="container mt-5">
        <h1 class="text-center">Gestión de "Lo Mejor Para Ti"</h1>
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <form action="admin_mejor_para_ti.php" method="POST" enctype="multipart/form-data">
            ...
            <tbody>
                <?php while ($elemento = $elementos->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <!-- Botón para Editar -->
                            <form action="admin_mejor_para_ti.php" method="POST" style="display:inline-block;">
