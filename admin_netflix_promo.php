...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "INSERT INTO InfoNetflix (descripcion, precio, imagen_url, boton_texto) VALUES ('$descripcion', '$precio', '$imagenPath', '$boton_texto')";
...
if (isset($_POST['editar'])) {
    $id = $_POST['id'];
    $resultado = $conexion->query("SELECT * FROM InfoNetflix WHERE id=$id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "UPDATE InfoNetflix SET descripcion='$descripcion', precio='$precio', imagen_url='$imagenPath', boton_texto='$boton_texto' WHERE id=$id";
        }
    } else {
        $query = "UPDATE InfoNetflix SET descripcion='$descripcion', precio='$precio', boton_texto='$boton_texto' WHERE id=$id";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar'])) {
    $id = $_POST['id'];
    $query = "DELETE FROM InfoNetflix WHERE id=$id";
    if ($conexion->query($query) === TRUE) {
...
$elementos = $conexion->query("SELECT * FROM InfoNetflix");
?>
...
<head>
    ...
    <title>Gestión de Netflix Promo</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
    <div class="container mt-5">
        <h1 class="text-center">Gestión de "Netflix Promo"</h1>
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <form action="admin_netflix_promo.php" method="POST" enctype="multipart/form-data">
            ...
            <tbody>
                <?php while ($elemento = $elementos->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><img src="<?php echo htmlspecialchars($elemento['imagen_url']); ?>" alt="Imagen de Netflix" style="width: 50px;"></td>
                        <td>
                            <!-- Botón para Editar -->
                            <form action="admin_netflix_promo.php" method="POST" style="display:inline-block;">
