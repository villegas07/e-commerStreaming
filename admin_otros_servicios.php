...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['crear'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "INSERT INTO OtrosServicios (nombre, precio, garantia, descripcion, imagen_url) VALUES ('$nombre', '$precio', '$garantia', '$descripcion', '$imagenPath')";
...
if (isset($_POST['editar'])) {
    $id = $_POST['id'];
    $resultado = $conexion->query("SELECT * FROM OtrosServicios WHERE id=$id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $imagenPath)) {
            $query = "UPDATE OtrosServicios SET nombre='$nombre', precio='$precio', garantia='$garantia', descripcion='$descripcion', imagen_url='$imagenPath' WHERE id=$id";
        }
    } else {
        $query = "UPDATE OtrosServicios SET nombre='$nombre', precio='$precio', garantia='$garantia', descripcion='$descripcion' WHERE id=$id";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar'])) {
    $id = $_POST['id'];
    $query = "DELETE FROM OtrosServicios WHERE id=$id";
    if ($conexion->query($query) === TRUE) {
...
$elementos = $conexion->query("SELECT * FROM OtrosServicios");
?>
...
<head>
    ...
    <title>Gestión de Otros Servicios</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
    <div class="container mt-5">
        <h1 class="text-center">Gestión de "Otros Servicios"</h1>
        ...
        <div class="card mb-4">
            <div class="card-body">
                <h5 class="card-title"><?php echo $editarElemento ? 'Editar Servicio' : 'Agregar Nuevo Servicio'; ?></h5>
                <form action="admin_otros_servicios.php" method="POST" enctype="multipart/form-data">
                    ...
                    <button type="submit" class="btn btn-primary"><?php echo $editarElemento ? 'Actualizar Servicio' : 'Crear Servicio'; ?></button>
                </form>
            </div>
        </div>
            ...
            <tbody>
                <?php while ($elemento = $elementos->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <!-- Botón para Editar -->
                            <form action="admin_otros_servicios.php" method="POST" style="display:inline-block;">
